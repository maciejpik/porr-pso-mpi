...
mypath = os.getcwd()
mypath = os.path.join(mypath, "wykresy","log_mpi_2d_task1")
...
# particlesBestArray = np.empty((0,cols), float)
# getBestSolutionHistory(arr,bestIds)  
particleTrj = getParticleHistory(iterSortAllData, 15, 100)
...
plt.title(f"{lib}, {alg}, {task}: n={dim}, cząstka o id:15, l.iteracji={100} - trajektoria")
plt.xlabel("x1")
plt.ylabel("x2")
cbar= plt.colorbar()
cbar.set_label("koszt")
plt.savefig(f"{lib}_ {alg}_{task}_trajectory_id15.{ext}")
